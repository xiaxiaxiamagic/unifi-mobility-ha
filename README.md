...
- 快速与慢速分组轮询，降低路由器负载
- 提供立即刷新、重新连接本地会话和重启路由器按钮
- 支持定位指示灯、PoE 直通、蜂窝模式、LTE 频段和 SIM 配置来源控制
...
- 状态轮询保持只读；只有用户操作控制实体时才调用本地写入接口。
- 重启、PoE、蜂窝模式、频段和 SIM 配置可能造成短暂断网，请谨慎用于自动化。
...
- Buttons for immediate refresh, local-session reconnection, and router restart
- Controls for locate, PoE passthrough, cellular mode, LTE band, and SIM profile source
...
- Polling is read-only; local write calls occur only when a user operates a control entity.
- Router restart, PoE, radio, band, and SIM-profile controls may briefly interrupt connectivity.
...
| UMR | Real firmware pending | Field aliases tested | Synthetic compatibility fixture |
